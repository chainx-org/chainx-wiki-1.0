...
2. 下载节点：
   ...
   **！！如果从零开始运行我们的节点，请先用 chainx-v0.9.10-x86_64-<....>-onchain-upgrade.tgz 版本同步到块高度 88200 以后，再换用 chainx-v0.9.10-x86_64-<....>-onchain-upgrade-20190518.tgz 升级到最新块高度。** 原因如下：

   由于在验证人换届排序时使用了不稳定排序，造成高度88200两个得票数相同的节点经排序后native和wasm执行不一致，所以如果使用了第一个发布的版本(无onchain-upgrade标识)同步时会卡在块高度88200。2019年5月18日下午的链上升级已经对该问题进行了修复，如果遇到同步到 88200 高度后无法继续同步，请确认是否先用了 onchain-upgrade 版本同步到最新，然后再用 onchain-upgrade-20190518 替换 onchain-upgrade 继续运行。
